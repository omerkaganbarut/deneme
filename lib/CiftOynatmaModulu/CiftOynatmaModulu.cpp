...
void coRun() {
  ...
  switch (durum) {
    ...
    {
      if (!moveToAktifMi(MOTOR_X) && !moveToAktifMi(MOTOR_B)) {
        ...
        Serial.println(F("    Y → Z referans pozisyonunu kaydet ve devam et"));
    ...
    case CO_Z_SIFIRLAMA_KONTROL:
    {
      if (Serial.available() > 0) {
        ...
        if (c == 'Y' || c == 'y') {
          ...
          // Z referans pozisyonunu kaydet
          ckMeta.zRefPos = zEnc->getPosition();
          
          Serial.println(F("  ✓ Z referans pozisyonu kaydedildi!"));
          Serial.print(F("     zRefPos = "));
          Serial.println(ckMeta.zRefPos);
          Serial.println(F("     (Encoder resetlenmedi, fiziksel pozisyon korundu)"));
    ...
    {
      if (!moveToAktifMi(MOTOR_Z) && !moveToAktifMi(MOTOR_X) && !moveToAktifMi(MOTOR_B)) {
        pulseAtDurdur(MOTOR_Z);
        pulseAtDurdur(MOTOR_X);
        pulseAtDurdur(MOTOR_B);
    ...
    case CO_K1_Z_ASAGI_BASLA:
    {
      ...
      // ORTAK FONKSİYON KULLAN
      long zHedef = oynatmaMapA0ToZEnc(a0Hedef);
